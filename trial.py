...
# Level 2
    ...
    # Level 3: The Gauntlet - Updated with new Trap Mechanic
    l3_walls = list(base_walls)
    # Zigzag walls for P1 (Gate obstacles)
    l3_walls.append(offset_rect((0, 240, 500, 20)))   # Top/Mid divider
    l3_walls.append(offset_rect((135, 480, 500, 20))) # Mid/Bot divider
    
    # Walls for P2 (Maze to navigate)
    l3_walls.append(offset_rect((800, 200, 20, 300)))
    l3_walls.append(offset_rect((1000, 100, 20, 200)))
    l3_walls.append(offset_rect((1000, 500, 20, 200)))

    l3_guard1_path = [offset_point((80, 480)), offset_point((80, 480))] # Blocking P1 bot->mid
    l3_guard2_path = [offset_point((550, 240)), offset_point((550, 240))] # Blocking P1 mid->top
    l3_guard_p2_path = [offset_point((900, 100)), offset_point((900, 600))] # P2 harasser

    levels.append({
        "name": "Level 3: The Gauntlet",
        "briefing": [
            "Beware of TRAP switches (Red)!",
            "If P2 triggers a trap: P1 FREEZES and P2 controls INVERT.",
            "To cure: P2 must reach the CYAN switch.",
            "If P2 hits a wall while trapped, they reset to start (still trapped)."
        ],
        "p1_start": offset_point((50, 650)), 
        "p2_start": offset_point((1200, 650)), 
        "key": offset_rect((580, 350, 40, 40)), # In middle section
        "chest": offset_rect((50, 50, 40, 40)), # In top section
        "walls": l3_walls,
        "guards": [
            # P1 Gatekeepers
            {"x": l3_guard1_path[0][0], "y": l3_guard1_path[0][1], "path": l3_guard1_path, "angle": 0, "id": 1, "speed": 0, "fov": 90, "len": 150, "color": C_GUARD_DEFAULT},
            {"x": l3_guard2_path[0][0], "y": l3_guard2_path[0][1], "path": l3_guard2_path, "angle": 180, "id": 2, "speed": 0, "fov": 90, "len": 150, "color": C_GUARD_DEFAULT},
        ],
        "deactivators": [
            # Real Switch for Gate 1
            {"x": offset_point((700, 100))[0], "y": offset_point((700, 100))[1], "id": 1, "fake": False, "color": C_DEACTIVATOR_DEFAULT},
            # Real Switch for Gate 2
            {"x": offset_point((700, 600))[0], "y": offset_point((700, 600))[1], "id": 2, "fake": False, "color": C_DEACTIVATOR_DEFAULT},
            
            # TRAP SWITCHES (ID 999) - Dark Red
            {"x": offset_point((1100, 400))[0], "y": offset_point((1100, 400))[1], "id": 999, "fake": False, "color": (139, 0, 0)},
            {"x": offset_point((900, 300))[0], "y": offset_point((900, 300))[1], "id": 999, "fake": False, "color": (139, 0, 0)},

            # CURE SWITCH (ID 888) - Cyan
            {"x": offset_point((1200, 50))[0], "y": offset_point((1200, 50))[1], "id": 888, "fake": False, "color": (0, 255, 255)},
        ],
        "instructions": [
             {"id": "l3_hint", "lines": ["Avoid Red Switches!", "Cyan cures the trap."], "rect": offset_rect((850, 30, 240, 60)), "start_active": True},
        ]
    })
