...
class TutorialInstruction:
    """Floating box for instructions."""
    def __init__(self, text_lines, rect):
        self.text_lines = text_lines
        self.rect = pygame.Rect(rect)
    
    def draw(self, surface):
        pygame.draw.rect(surface, C_TUTORIAL_BOX, self.rect, border_radius=8)
...
def get_levels():
    ...
    l0_walls.extend([offset_rect((150, 100, 20, 600)), offset_rect((450, 0, 20, 520))])
...
class Game:
    ...
    def load_level(self, idx, initial_load=False):
        ...
        self.sync_zones = []
        for sz_data in data.get("pressure_plates", []):
        ...
        for i, dw_data in enumerate(data.get("dynamic_walls", [])):
            ...
            if self.current_level_idx == 2: 
                if dw.link_id == 10: dw.is_open = False 
        ...
        for instr in data.get("instructions", []):
            self.tutorial_instructions.append(TutorialInstruction(instr["lines"], instr["rect"]))
    ...
    def update(self):
        ...
        if self.state == "MAIN_MENU":
            ...
            for sz in self.sync_zones:
                ...
                if sz.is_active: active_links[sz.link_id] = True
